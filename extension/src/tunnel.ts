...
export class TunnelManager {
  ...
  async ensureCloudflared(): Promise<string> {
    ...
    this.out.appendLine('cloudflared 다운로드 중...');
    await download(cfDlUrl(), dest);
    this.out.appendLine('cloudflared 다운로드 완료.');
    return dest;
  }
  ...
  async start(mode: string, port: number, namedName?: string, namedUrl?: string): Promise<void> {
    ...
    this.out.appendLine(`터널 시작 중 (포트 ${port})...`);
  ...
  async showQR(url: string): Promise<void> {
    ...
    const bar = '─'.repeat(52);
    this.out.appendLine(`\n${bar}`);
    this.out.appendLine(`  접속 URL: ${url}`);
    this.out.appendLine('');
    this.out.appendLine(ascii);
    this.out.appendLine('  휴대폰으로 QR을 찍어 접속하세요');
    this.out.appendLine(`${bar}\n`);
    this.out.show(true);
  }
